...
const csvFilename = 'income-poverty-estimates.csv';
...
function interrogateSPARQL(zones) {
    ...
    _.forEach(_.chunk(zones, chunk_size), (zone_chunk) => {
        queries.push(`
            ...
            WHERE {
                ?s qb:dataSet data:income-and-poverty-modelled-estimates ;
                   qb:measureType ?m ;
                   ?m ?value ;
                   <http://statistics.gov.scot/def/dimension/indicator(incomeAndPovertyModelledEstimates)> ?i ;
                   ldim:refArea ?z ;
                   ldim:refPeriod ?y .
                ?z rdfs:label ?zone .
